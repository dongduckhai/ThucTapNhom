        ...
        <div class="card-body">
            <form method="POST" action="{{url('admin/product/store')}}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="brand_id" id="brand"class="form-control" style="width:25%">
                        ...
                        @foreach ($brand_list as $brand)
                            <option value="{{$brand->id}}" {{ old('brand_id') == $brand->id ? ' selected = "selected" ' : ''}}>
                                {{$brand->name}}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control w-50" type="text" name="old_price" id="old_price" value="{{old('old_price')}}">
                @error('old_price')
                    <small class="text-danger font-italic">{{$message}}</small>
                @enderror
                </div>
                <div class="form-group">
                    ...
                    <textarea name="desc" class="form-control ckeditor" id="desc" cols="30" rows="15">{!! old('desc') !!}</textarea>
                @error('desc')
                    <small class="text-danger font-italic">{{$message}}</small>
                @enderror
                </div>
                <div class="form-group">
                    ...
                    <textarea name="details" class="form-control ckeditor" id="details" cols="30" rows="15">{!! old('details') !!}</textarea>
                @error('details')
                    <small class="text-danger font-italic">{{$message}}</small>
                @enderror
                </div>
